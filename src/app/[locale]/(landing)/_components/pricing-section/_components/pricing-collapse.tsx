...
export default function PricingCollapse({ data }: { data: TPackage }) {
	...
	return (
		<Disclosure as={Menu.Item} className='overflow-hidden rounded-md outline-none'>
			<>
				<Disclosure.Button className='z-10 flex w-full items-center justify-between border-none p-6 text-gray-800 outline-none ui-open:bg-gray-50 hover:bg-gray-50 focus:border-none'>
						...
						{data.appendixes.map((item: any, index: number) => {
							return (
								<Menu.Item
									as='div'
									key={index}
									className='grid h-20 grid-cols-[10%_90%] items-center rounded-sm border-none py-2 pl-10 pr-2 outline-none transition-all duration-200 ease-in-out hover:bg-gray-200 focus:border-none md:grid-cols-[5%_95%]'>
